        ...
        window.fetchCollegesFromFirestore = async function(state, district) {
            try {
                const collegesRef = collection(db, 'colleges');
                const q = query(collegesRef, where("state", "==", state), where("district", "==", district));
        ...
        window.populateColleges = async function() {
            ...
            if (window.fetchCollegesFromFirestore) {
                const colleges = await window.fetchCollegesFromFirestore(state, district);
                colSelect.innerHTML = '<option value="" disabled selected>Select College</option>';
                colleges.sort((a,b) => a.name.localeCompare(b.name)).forEach(c => {
                    const opt = document.createElement('option');
                    opt.value = c.name;
